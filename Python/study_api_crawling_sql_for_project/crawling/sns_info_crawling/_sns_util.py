...
def convert_tiktok_all_data(df : pd.DataFrame):
    ...
    ## return 변경된 df
    '''
    view = [f'view{i}' for i in range(1, 6)]
    like = [f'like{i}' for i in range(1, 6)]
    comments = [f'comments{i}' for i in range(1, 6)]
    contents = [f'contents{i}' for i in range(1, 6)]
    df.columns = ['channeltype', 'category', 'area', 'channelid', 'channelname', 'channelurl', 'following', 'follower', 'total_like', 'description', "view", "postdate"] + view + like + comments + contents + ["extra", "imageurl", "contact"]
